...
template <int NDim, int Dim=1>
struct sum_expr_loop;

template <int NDim>
struct sum_expr_loop<NDim, NDim>
{
    template <typename RHS>
    auto operator()(RHS& rhs, const std::array<len_type, NDim>& len) const
    {
        detail::decay_t<decltype(eval(rhs))> val{};
...
struct sum_expr_loop
{
    ...
    auto operator()(RHS& rhs, const std::array<len_type, NDim>& len) const
    {
        sum_expr_loop<NDim, Dim+1> next_loop;

        detail::decay_t<decltype(next_loop(rhs, len))> val{};

        for (len_type i = 0;i < len[Dim-1];i++)
        {
            val += next_loop(rhs, len);
...
template <typename Expr, typename = detail::enable_if_t<is_expression_arg<detail::decay_t<Expr>>::value>>
auto sum(Expr&& expr_)
{
    typedef typename expression_type<detail::decay_t<Expr>>::type expr_type;
    constexpr int NDim = expr_dimension<expr_type>::value;

    auto expr = make_expression(std::forward<Expr>(expr_));

    auto len = get_expr_lengths<NDim>(expr);
    for (auto i : range(NDim))
        MARRAY_ASSERT(len[i] != -1);

    return sum_expr_loop<NDim>()(expr, len);
}
